...
int main(int argc, char **argv)
{
    ...
    app.setStyle("Breeze");
    QCoreApplication::setApplicationName(QString("KELO Drive Data Viewer"));
    ...
    if (!input_source.empty())
    {
        gui.selectSource(input_source);
    }
    if (!network_interface.empty())
    {
        gui.selectNetworkInterface(network_interface);
    }
    if (!config_file.empty())
    {
        gui.setConfigFile(config_file);
    }
    if (!pcap_file.empty())
    {
        gui.setPCAPFile(pcap_file);
    }
    if (publish_zmq)
    {
        gui.enableZMQ(true);
